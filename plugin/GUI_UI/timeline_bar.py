...
class parts:
    def UI_set(self, UI_operation):
        ...
        def click_drag(event):
            ...
            # data.operation["log"].write(motion["x"])

            #data.operation["log"].write(motion, touch, canvas_within)
        ...
        def motion(event):
            ...
            data.operation["log"].write(data.first_touch, this_touch)
            target = [this_touch.get("left"), this_touch.get("right")]

            # data.operation["log"].write("移動検知")
