...
class TextReceivePoint:
    ...
    def text_func(self, text):
        old_text_data = self.data.all_data.get_key_frame_val(self.media_id, self.effect_id, self.effect_uuid_key, self.mov_key)
...
class TextReceiveVariousFixed:
    ...
    def text_func(self, text):
        old_s = str(self.data.all_data.edit_various_fixed(self.media_id, self.effect_id, self.various_fixed_key))
        now_s = str(text)
